...
### Legacy Endpoint: `GET /api/wba-verify`

Original verification endpoint with hardcoded RFC 9421 test keys (for backwards compatibility).

## Test Keys

The project uses RFC 9421 test keys from the official specification:
...
Source: [RFC 9421 Appendix B.1.4](https://datatracker.ietf.org/doc/html/rfc9421#appendix-B.1.4)
...
wba-quickstart/
├── api/
│   ├── verify.ts               # Main verification endpoint (trust-on-first-use)
│   ├── wba-verify.ts           # Legacy endpoint with hardcoded keys
│   ├── directory.ts            # Directory fetching and caching
│   ├── types.ts                # TypeScript type definitions
│   └── test-directory.ts       # Mock directory for testing
├── public/
│   └── index.html              # Frontend interface
├── tests/
│   ├── test-verification.ts    # Unit tests
│   ├── test-signed-request.ts  # Valid signature tests
│   └── test-unsigned-requests.ts # Error scenario tests
├── package.json
├── tsconfig.json
...
See `test-signed-request.ts` for a complete example.
...
Replace hardcoded test keys with dynamic key retrieval:

```typescript
// Fetch public key from your registry
const publicKey = await fetchPublicKeyFromRegistry(keyId);
...
- `DEBUGGING.md` - VSCode debugging guide for the verification API
- `KEY_DOWNLOAD.md` - Public key download implementation details
- `WBA_IMPLEMENTATION.md` - Detailed implementation guide with examples
- [RFC 9421](https://datatracker.ietf.org/doc/html/rfc9421) - HTTP Message Signatures
- [web-bot-auth Package](https://www.npmjs.com/package/web-bot-auth)
- [Cloudflare Examples](https://github.com/cloudflare/web-bot-auth/tree/main/examples)

## Security Considerations

1. **Test Keys Only**: RFC 9421 keys are for testing. Generate your own for production.
2. **HTTPS Required**: Always use HTTPS in production.
3. **Key Rotation**: Implement key rotation and versioning.
4. **Timestamp Validation**: Signatures include created/expires times (enforced by package).
5. **Public Key Registry**: In production, fetch public keys from a trusted registry.

## Troubleshooting

### "Signature verification failed"

- Verify you're using the correct key ID (`test-key-ed25519` for test key)
- Ensure URL in signature matches request URL exactly (including scheme)
- Check signature hasn't expired (5-minute window by default)
- Confirm all signed components are present in the request
